...
class GameViewModel {
    ...
    private func advanceCourtesyTurnPastBots() {
        ...
        for i in 0..<players.count where charlestonPendingPasses[i] == nil && players[i].isBot {
            ...
            // STEP 3 (seat-ownership): host filled this bot seat's pass.
            bumpSeatRevision(i)
        }
    ...
    private func finalizeSoloCourtesyExchange() {
        ...
        for i in 0..<players.count {
            ...
            // STEP 3 (seat-ownership): the host just mutated this seat
            // authoritatively, so its slice is now newer than whatever that
            // client holds. Bumping makes the exchange win the comparison.
            bumpSeatRevision(receiverIdx)
        }
        for i in 0..<players.count where players[i].isBot {
    ...
    func forceFinalizeStuckCharleston(reason: String) -> Bool {
        ...
        for i in 0..<players.count where players[i].isBot && charlestonPendingPasses[i] == nil {
            ...
            // STEP 3 (seat-ownership): host auto-picked for this stalled seat.
            bumpSeatRevision(i)
            autoFilled.append(i)
        }
        ...
        if isCourtesy {
            ...
            for i in 0..<players.count {
                ...
                let tiles = charlestonPendingPasses[i] ?? []
                players[receiverIdx].hand.append(contentsOf: tiles)
                // STEP 3 (seat-ownership): authoritative host mutation of this seat.
                bumpSeatRevision(receiverIdx)
            }
            for i in 0..<players.count where players[i].isBot {
                players[i].hand.sort { sortTile($0) < sortTile($1) }
    ...
    private func executeCall(playerIndex: Int, type: CallType, discarded: MahjongTile) {
        ...
        // STEP 3 (seat-ownership): the exposure changed this seat's hand and
        // exposedSets. When the host runs it for a remote player, that seat's
        // slice is now authoritative and must outrank the client's own copy.
    ...
    private func executeBotTurn() {
        ...
        // STEP 3 (seat-ownership): host drew for this bot seat.
        bumpSeatRevision(botIdx)
    ...
    private func executeBotDiscard(botIdx: Int) {
        ...
        // STEP 3 (seat-ownership): host-driven mutation of the bot's seat.
        bumpSeatRevision(botIdx)
    ...
    private func hostForceAdvanceRemoteSeat(seat: Int) {
        ...
        if !hasDrawnThisTurn {
            ...
            // STEP 3 (seat-ownership): the host drew on behalf of this stalled
            // seat — that mutation must outrank the client's stale copy.
            bumpSeatRevision(seat)
            hasDrawnThisTurn = true
            invalidMahjongMessage = nil
            callAvailable = false
        ...
        // STEP 3 (seat-ownership): host discarded on behalf of this seat.
        bumpSeatRevision(seat)
        tile.isDiscarded = true
        discardPile.append(tile)
        ...
        if isOnlineMode {
            ...
            if weArePlaying && incomingStatus == .playing
                ...
                let isCallerExposure: Bool = {
                    guard state.discardPile.count == discardPile.count - 1 else { return false }
                    var callerIdx: Int?
