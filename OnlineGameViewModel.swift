...
class OnlineGameViewModel {
    ...
    /// Throttle for the "straggler pull-forward" re-push: when we're already
    /// playing but a peer is still broadcasting charleston, we re-push our full
    /// authoritative state to drag them forward. Bounded so a chatty straggler
    /// can't turn each of its echoes into one of our broadcasts.
    private var lastStragglerRepushAt: Date = .distantPast
    ...
    private func handleStateUpdateBroadcast(_ event: JSONObject, gameViewModel: GameViewModel) async {
        ...
        do {
            ...
            // STRAGGLER PULL-FORWARD. If we're already PLAYING but this peer is
            // still broadcasting CHARLESTON, it missed the one-shot charleston→
            // playing transition. applyRemoteState correctly ignored their stale
            // charleston state, but the fix has to be active: since the play-phase
            // heartbeat now sends stateless pings (which can't carry a phase
            // transition), re-push our full authoritative playing state so the
            // straggler can actually advance. Without this, a peer that dropped the
            // single transition broadcast is stranded in charleston forever,
            // flip-flopping on the courtesy pointer while the host plays on — the
            // exact "invitee glitching on courtesy pass, game stuck after courtesy"
            // report. Throttled, and self-limiting: once they apply our state they
            // stop echoing charleston, so this stops firing.
            if gameViewModel.gameStatus == .playing,
               !gameViewModel.showEndGameOverlay,
               payload.status == OnlineGameStatus.charleston.rawValue,
               payload.senderSeat != gameViewModel.localSeatIndex,
               let gameId = currentGameId,
               Date().timeIntervalSince(lastStragglerRepushAt) > 1.5 {
                lastStragglerRepushAt = Date()
                let state = gameViewModel.serializeState()
                print("⏩ pulling straggler seat \(payload.senderSeat) forward — re-pushing playing state (they were still charleston)")
                await broadcastStateUpdate(
                    gameId: gameId,
                    status: OnlineGameStatus.playing.rawValue,
                    state: state,
